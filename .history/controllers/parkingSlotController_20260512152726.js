...
exports.createParkingSlot = async (req, res) => {
  try {
    ...
    // Only add slotNumber if provided (otherwise auto-generate)
    if (slotNumber && slotNumber.trim() !== '') {
      ...
      if (existingSlot) {
        return res.status(400).json({
          success: false,
          message: `Parking slot ${slotNumber.toUpperCase()} already exists`
        });
      }
      slotData.slotNumber = slotNumber.toUpperCase();
    }
...
exports.createBulkParkingSlots = async (req, res) => {
  try {
    ...
    const letterMatch = startFrom.match(/[A-Za-z]/);
    const numberMatch = startFrom.match(/\d+/);
    
    const startLetter = letterMatch ? letterMatch[0] : 'A';
    const startNumber = numberMatch ? parseInt(numberMatch[0]) : 1;
...
exports.updateParkingSlot = async (req, res) => {
  try {
    ...
    if (slotNumber && slotNumber.trim() !== '') {
      ...
      updateData.slotNumber = slotNumber.toUpperCase();
    }
